...
def employer_from_signup(params):
    ...
    employer = Employer(contact_name=contact_name, company_name=company_name, email=email, pwd=pwd)
    return employer
...
def add_employer_offer(employer, params):
    ...
    o = EmployerOffer(employer_id=employer.id,
              ...
              interview_details=params.get('interview_details'),
...
EMPLOYER_EDITABLES = {'company_name': ID, 'website': ID, 'address_line1': ID, 'address_line2': ID, 'address_line3': ID,
                      'address_zipcode': ID, 'address_city': get_location_by_name_or_raise, 'contact_name': ID,
                      'contact_phone': ID, 'contact_email': ID, 'contact_position': ID, 'logo_url': ID, 'fb_url': ID,
                      'linkedin_url': ID, 'image_video_url': ID, 'mission_text': ID, 'culture_text': ID,
                      'vision_text': ID, 'founding_year': ID, 'revenue_pa': ID, 'funding_amount': ID,
                      'funding_text': ID, 'no_of_employees': ID, 'tech_team_size': ID, 'tech_team_philosophy': ID,
                      'recruitment_process': ID, 'training_policy': ID,
