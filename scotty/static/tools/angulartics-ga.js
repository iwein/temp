...
.config(['$analyticsProvider', function ($analyticsProvider) {
  ...
  $analyticsProvider.registerPageTrack(function(path) {
    if ($analyticsProvider.prefix)
      path = $analyticsProvider.prefix + path;

    if (window._gaq) window._gaq.push(['_trackPageview', path]);
    if (window.ga) {
      window.ga('send', 'pageview', path);
      angular.forEach($analyticsProvider.settings.ga.additionalAccountNames, function (accountName){
        window.ga(accountName +'.send', 'pageview', path);
      });
    }
  });
   ...
   * @param {object} properties Comprised of the mandatory field 'category' (string) and optional
   *    fields 'label' (string), 'value' (integer) and 'noninteraction' (boolean)
   *
  ...
  $analyticsProvider.registerEventTrack(function (action, properties) {
    ...
    if (window.ga) {
      ...
      angular.forEach($analyticsProvider.settings.ga.additionalAccountNames, function (accountName){
        window.ga(accountName +'.send', 'event', eventOptions);
      });
    }

    else if (window._gaq) {
      window._gaq.push(['_trackEvent', properties.category, action,
